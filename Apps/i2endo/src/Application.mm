...
#define dX 640.0
#define dY 640.0
    CGRect r = CGRectMake(cX - dX/2, cY - dY/2, dX, dY);
    CIImage *tmpImg = [img[0] imageByCroppingToRect:r];
    
    CIFilter* transform = [CIFilter filterWithName:@"CIAffineTransform"];
    NSAffineTransform* affineTransform = [NSAffineTransform transform];
    [affineTransform translateXBy:-(cX - dX/2) yBy:-(cY - dY/2)];
    [transform setValue:affineTransform forKey:@"inputTransform"];
    [transform setValue:tmpImg forKey:@"inputImage"];
    CIImage* tmpImg2 = [transform valueForKey:@"outputImage"];
    CIImage* imgOut;
    if (nCameras >= 2 && captures[1]->isReady()) {
        imgOut = [mChromakey updateFilter:img[1] withBkgnd:(mStatus.adjustChromaMode)? nil : tmpImg2];
    } else {
        imgOut = tmpImg2;
    }
    dispatch_async(mMainQueue, ^{
        emit(reqUpdateImages(imgOut));
    });
...
void Application::initialize(void)
{
    ...
    for (int i=0; i<nCameras; i++) {
        Cap::CaptureType type = Cap::kCaptureTypeUndefined;
        Cap::Capture *cap = nullptr;
