...
class EndoView : public QCvOGLWidget
{
	Q_OBJECT

public:
	//EndoView(QWidget* parent = nillptr);
    EndoView(QWidget* parent = 0, const QOpenGLWidget * shareWidget = 0, Qt::WindowFlags f = 0);
	virtual	~EndoView() override;

    void setCircleMaskVisibility(bool vis);
    bool circleMaskVisibility(void);

    void setCircleMaskXShiftRatio(float rx);
    float circleMaskXShiftRatio(void);
    void setCircleMaskYShiftRatio(float ry);
    float circleMaskYShiftRatio(void);
...
private:
    ...
    void getCircleParameters(int *cx, int *cy, int *radius);
    
    float maskXShiftRatio, maskYShiftRatio;      /// shift of circle center, relative to width/height.
